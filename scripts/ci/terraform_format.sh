...
then
    ...
    cd $TEST_DIR
    echo "concourse-ci terraform-format test failed on this pull request's commit: $(git rev-parse HEAD | cut -c1-7). Please run \`terraform fmt\`." > ../pull-request-comment/comment
    cd ../
else
